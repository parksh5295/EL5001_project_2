...
COARSE_WEAK_OUT="results/events_weak_labeled_coarse.ndjson"
STREAM_OUT="results/stream_events_coarse.ndjson"
NUM_STREAMS="100000"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --coarse-weak-out) COARSE_WEAK_OUT="$2"; shift 2 ;;
    ...
    --coarse-mode) COARSE_MODE="$2"; shift 2 ;;
...
if [[ "$SKIP_BUILD" != "true" ]]; then
  ...
  echo "[2/4] Build coarse attack groups -> ${COARSE_WEAK_OUT} (mode=${COARSE_MODE})"
  "${PIPENV[@]}" run python -m threat_agent.build_coarse_attack_events \
    --input "$WEAK_LABELED_OUT" \
    --output "$COARSE_WEAK_OUT" \
    --summary-json results/events_coarse_summary.json \
    --mode "$COARSE_MODE"

  echo "[3/4] Build coarse stream episodes -> ${STREAM_OUT}"
  "${PIPENV[@]}" run python -m threat_agent.stream_builder \
    --input "$COARSE_WEAK_OUT" \
    --output "$STREAM_OUT" \
    --summary-json results/stream_summary_coarse.json \
  ...
  echo "[skip] Reusing existing coarse stream data -> ${STREAM_OUT}"
fi

echo "[4/4] Run coarse stream comparison experiments"
"${PIPENV[@]}" run python -m threat_agent.stream_experiment_compare \
  ...
  --output-json "results/stream_compare_summary_coarse.json" \
  --output-csv "results/stream_compare_summary_coarse.csv"

echo "Done."
echo "Summary JSON: results/stream_compare_summary_coarse.json"
echo "Summary CSV : results/stream_compare_summary_coarse.csv"
